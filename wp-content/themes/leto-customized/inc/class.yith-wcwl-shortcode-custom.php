...
if ( ! defined( 'YITH_WCWL' ) ) {
	error_log('yith wcwl not defined');
	exit;
} // Exit if accessed directly


error_log('custom yith run');
if( ! class_exists( 'YITH_WCWL_Shortcode_custom' ) ) {
	/**
	 * YITH WCWL Shortcodes
	 *
	 * @since 1.0.0
	 */
	class YITH_WCWL_Shortcode_custom {
		...
		public static function init() {
			...
			if ( shortcode_exists('yith_wcwl_wishlist') ) {
				error_log('yith_wcwl_wishlist exists and removed');
				remove_shortcode( 'yith_wcwl_wishlist' );	
			}
			else {
				error_log('yith_wcwl_wishlist not foundd');
			}
			
			if ( shortcode_exists('yith_wcwl_add_to_wishlist') ) {
				error_log('yith_wcwl_add_to_wishlist exists and removed');
				remove_shortcode( 'yith_wcwl_add_to_wishlist' );	
			}

			// register shortcodes.
			add_shortcode( 'yith_wcwl_wishlist', array( 'YITH_WCWL_Shortcode_custom', 'wishlist' ) );
			add_shortcode( 'yith_wcwl_add_to_wishlist', array( 'YITH_WCWL_Shortcode_custom', 'add_to_wishlist' ) );

			// register gutenberg blocks.
			add_action( 'init', array( 'YITH_WCWL_Shortcode_custom', 'register_gutenberg_blocks' ) );
			add_action( 'yith_plugin_fw_gutenberg_before_do_shortcode', array( 'YITH_WCWL_Shortcode_custom', 'fix_for_gutenberg_blocks' ), 10, 1 );

			// register elementor widgets.
			add_action( 'init', array( 'YITH_WCWL_Shortcode_custom', 'init_elementor_widgets' ) );
		}
		...
		public static function init_elementor_widgets() {
			...
			// register widgets.
			add_action( 'elementor/widgets/widgets_registered', array( 'YITH_WCWL_Shortcode_custom', 'register_elementor_widgets' ) );
		}
...
YITH_WCWL_Shortcode_custom::init();
