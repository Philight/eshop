...
// ion-android-checkmark-circle, ion-android-done
			...
			this.messages = {
				...
				error: {
					...
					USER_TAKEN 		: '<strong>Username</strong> is already taken. Please choose another one.', 
		...
		showNotifications(){
			...
			var $noticeCont = this.$page.find('.xoo-wsc-notice-container');
	...
	class RegistrationPage {
		...
		eventHandlers() {
			...
			this.$page.on( 'change', '#BillingOption', this.toggleBilling.bind(this) );
...
//---  COUNTRY CODE
		toggleCountryCodeDropdown(e) {
			var $dropdown = $(e.currentTarget).parent().siblings('.country-code-dropdown');
...
*/
		...
		validateAllFields() {
			...
			// REQUIRED FIELDS
			var $profileInputs = this.$page.find('.register-profile input[required]');
			var $shippingInputs = this.$page.find('.register-shipping input[required]');
			
			var $billingInputs = null;
			if (!this.billingSameAsShipping) {
				$billingInputs = this.$page.find('.register-billing input[required]');
			}
			
			var $allInputs = $profileInputs.add($shippingInputs).add($billingInputs);
			...
			// OPTIONAL FIELDS
			$profileInputs = this.$page.find('.register-profile input:not(:required)');
			$shippingInputs = this.$page.find('.register-shipping input:not(:required)');
			$billingInputs = null;
			if (!this.billingSameAsShipping) {
				$billingInputs = this.$page.find('.register-billing input[type=text]:not(:required)');
			}
			var $optionalInputs = this.$page.find('.register-optional input:not(:required)');

			$allInputs = $profileInputs.add($shippingInputs).add($billingInputs).add($optionalInputs);
...
//---END VALIDATION
		...
		submitFormData(e) {
			...
			if ( anyInvalidField ) {
				...
				//this.notice.showNotifications;
				$("html, body").animate({ scrollTop: 0}, 'slow');

				return;
			} else {
				this.notice.add( 'success', 'ACC_CREATED' );
			...
			var formData = this.$page.children('.register-form').serialize();
